...
cv_clf = linear_model.LogisticRegression(C=1e4, solver='sag')                             
...
for train, test in skf.split(X,y): 
    ...
    print('LOGIS:acc=%f,precision=%f,npv=%f,sensitivity=%f,specificity=%f,mcc=%f,f1=%f,roc_auc=%f'
...
yscore_sum.to_csv('Lyscore_sum.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('Lytest_sum.csv')
fpr, tpr, _ = roc_curve(ytest[:,0], yscore[:,0])
auc_score=np.mean(scores, axis=0)[7]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='L ROC (area = %0.2f%%)' % auc_score)
